 ...
 */
/*
  Simulator for the Ainstein LR-D1 Serial rangefinder - with v19.0.0 firmware
*/

#include "SIM_RF_Ainstein_LR_D1_v19.h"

using namespace SITL;

uint32_t RF_Ainstein_LR_D1_v19::packet_for_alt(float alt_m, uint8_t *buffer, uint8_t buflen)
{
    ...
    if (alt_m*100 > 65535) {
        malfunction_alert |= 1U << 10;  // AltitudeReading alert
    }

    const uint16_t alt_cm = alt_m * 100;  // this really does wrap on real hardware
    ...
    buffer[5] = LOWBYTE(malfunction_alert);  // object count
